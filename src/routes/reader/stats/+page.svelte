<script lang="ts">
  ...
  async function register() {
    ...
    try {
      const res = await fetch(`${API}/sync/register`, {
        ...
        body: JSON.stringify({ username: syncUsername, pin: syncPin }),
      });
      ...
      syncStatus = 'error';
      syncMessage = 'Server nicht erreichbar';
    }
  }
  ...
  async function pushToServer() {
    ...
    try {
      const res = await fetch(`${API}/sync/push`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username: syncUsername, pin: syncPin, log }),
      });
      const data = await res.json();
      if (!res.ok) { syncStatus = 'error'; syncMessage = data.error; return; }
      syncStatus = 'success';
      syncMessage = `${log.length} Einträge hochgeladen ✓`;
    } catch {
      syncStatus = 'error';
      syncMessage = 'Server nicht erreichbar';
    }
  ...
  async function pullFromServer() {
    ...
    try {
      ...
      saveLog();
      syncStatus = 'success';
      syncMessage = `${log.length} Einträge heruntergeladen ✓`;
    } catch {
      syncStatus = 'error';
      syncMessage = 'Server nicht erreichbar';
    }
  ...
  $: daySums = (() => {
    ...
    for (const e of log) {
      ...
      if (existing) {
        ...
        map.set(e.date, {
          date: e.date,
          rsvp: e.source === 'rsvp' ? e.words : 0,
          manual: e.source === 'manual' ? e.words : 0,
          total: e.words,
          label: '',
        });
      }
...
<main>
  ...
  <div class="hero">
    ...
    <div class="cards">
      ...
      <div class="card">
        <span class="card-icon">📆</span>
        <span class="card-value">{fmtWords(weekTotal)}</span>
        <span class="card-label">Diese Woche</span>
      </div>
      <div class="card">
        <span class="card-icon">🏆</span>
    ...
    <div class="tabs">
      ...
      <button class="tab" class:active={activeTab === 'sync'}  on:click={() => activeTab = 'sync'}>
        {isLoggedIn ? '☁️ Sync' : '☁️ Sync'}
      </button>
    </div>
    ...
    {#if activeTab === 'chart'}
      <div class="chart-box">
        <p class="chart-title">Wörter diese Woche</p>
        {#if maxWords <= 1}
          <p class="empty-hint">Noch keine Lesedaten. Lese etwas im RSVP Reader oder trage manuell ein.</p>
        {:else}
          <div class="bar-chart">
            {#each chartDays as day}
              <div class="bar-col">
                ...
                <div class="bar-track">
                  <div class="bar-wrap" style="height:{(day.total / maxWords) * 100}%">
      ...
      {#if daySums.length > 0}
        <div class="chart-box">
          <p class="chart-title">Alle Einträge</p>
      ...
      <div class="chart-box">
        ...
        {:else}
          ...
          <div class="sync-user">
            ...
            <div>
              <p class="sync-name">{syncUsername}</p>
              <p class="sync-sub">Angemeldet</p>
            </div>
          ...
          <div class="sync-actions">
            <button class="sync-btn upload" on:click={pushToServer} disabled={syncStatus === 'loading'}>
              ...
              <div>
                <p>Hochladen</p>
                <p class="sync-btn-sub">Lokale Daten → Server</p>
              </div>
            </button>
            <button class="sync-btn download" on:click={pullFromServer} disabled={syncStatus === 'loading'}>
              ...
              <div>
                <p>Herunterladen</p>
                <p class="sync-btn-sub">Server → Dieses Gerät</p>
              </div>
            </button>
          </div>

          {#if syncStatus === 'loading'}
            <p class="sync-msg">Lädt…</p>
...
<style>
  ...
  .tab { flex: 1; border: none; background: transparent; border-radius: 12px; padding: 10px; font-size: 0.95rem; font-weight: 600; color: #bbb; cursor: pointer; transition: background 0.15s, color 0.15s; }
  ...
  .chart-title { font-size: 0.82rem; font-weight: 700; color: #bbb; text-transform: uppercase; letter-spacing: 0.7px; text-align: left; }
